...
`fleurs-nl-batch` already targets — no collision to work around anymore.
When a profile matches more than one pack, `_choose_packs_for_profile()`
now prompts (checkbox, `fleurs-nl-batch` pre-checked as the old default)
...
`goesb run whisper-medium-nl-batch common-voice-nl-elderly-batch`
