...
oesb --help
oesb env          # print captured environment fingerprint
oesb validate profiles/whisper-medium-en-batch/profile.yaml
oesb validate packs/example-librispeech-en-batch/pack.yaml
...
oesb run whisper-medium-en-batch example-librispeech-en-batch --repeats 2
```

Pass `--model-override tiny` (or `base`) to `oesb run` for a fast local smoke
